...
const AgentSession = ({ history, isProcessing }: AgentSessionProps) => {
  const formatHistoryLine = (item: AgentHistoryItem) => {
    if (item.type === 'question') {
      return { label: 'Question', content: item.content };
    }
    return { label: 'Answer', content: item.content };
  };
  ...
  return (
    <Box flexGrow={1} flexDirection="column" flexShrink={0} flexBasis="75%" overflowY="visible">
      {history.map((item, index) => {
        ...
        const isNewUserQuestion = label === 'Question' && index > 0 && formatHistoryLine(history[index - 1]).label === 'Answer';
